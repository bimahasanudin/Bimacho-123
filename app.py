from flask import Flask, redirect, render_template, url_for, redirect, session, request,  flash, get_flashed_messages
...
def mobil():
    ...
    #     no_polis = request.form['no_polis']
    #     status = request.form['status']
    #     merk = request.form['merk']
    #     price_km = request.form['price_km']
    ...
    #             cursor.execute("INSERT INTO detail_mobil (no_polis, status, merk, price_km, image) VALUES (%s, %s, %s, %s, %s)", 
    #                            (no_polis, status, merk, price_km, filename))
...
def daftar_driver():
    ...
    #     cursor = db.cursor()
    #     cursor.execute("INSERT INTO keranjang (item, sewa_per_hari, id_pelanggan) VALUES (%s, %s, %s)", 
    #                            (driver, sewa_per_hari, id_pelanggan))
    #     db.commit()
    #     cursor.close()
    #     flash('Registration successful!','success')
    cursor = db.cursor()
...
#     id_pelanggan = session['id_pelanggan']
#     if request.method == 'POST':
#         id_driver = request.form['driver']
#         id_order = request.form['id_order']
#         cursor = db.cursor()
#         cursor.execute("select * from driver where id_driver=%s",(id_driver,))
#         nilai = cursor.fetchone()
#         cursor.execute("UPDATE keranjang set id_driver=%s, name=%s, driver_per_hari=%s where id_order=%s", 
...
#     drivers = cursor.fetchall()
#     cursor= db.cursor()
#     cursor.execute('SELECT sum(mobil_per_hari+driver_per_hari) FROM keranjang where id_pelanggan=%s', (id_pelanggan,))
#     sewa_per_hari = cursor.fetchone()
#     return render_template('keranjang.html', keranjang_item = item, sewa_per_hari = sewa_per_hari, drivers = drivers )
...
def transaksi():
    ...
    cursor = db.cursor()
    cursor.execute('SELECT transaksi.id_order, daftar_mobil.merk, driver.name, transaksi.tanggal_mulai,durasi,total,status FROM transaksi join daftar_mobil on transaksi.kode_mobil=daftar_mobil.kode_mobil left join driver on transaksi.id_driver = driver.id_driver WHERE id_pelanggan = %s', (id_pelanggan,))
    item = cursor.fetchall()

    # cursor.execute('SELECT * FROM driver')
    # drivers = cursor.fetchall()

    # cursor.execute('SELECT SUM(mobil_per_hari + driver_per_hari) FROM transaksi WHERE id_pelanggan = %s', (id_pelanggan,))
    # sewa_per_hari = cursor.fetchone()
...
def pesanan():
    ...
    if request.method == 'POST':
        ...
        for kolom in isidata:
            ...
            print(f'TOTAL {totalhari*durasi_int}')
            cursor = db.cursor()
            cursor.execute("INSERT INTO transaksi (id_order, id_pelanggan, kode_mobil, id_driver, tanggal_mulai, durasi, total, status) VALUES (%s, %s, %s, %s, %s, %s, %s, 'Menunggu Persetujuan')", 
                               (kolom[0], id_pelanggan, kolom[2], kolom[5], tanggal_mulai, durasi, totalhari * durasi_int,))
            db.commit()
...
def transaksi_selesai():
    cursor = db.cursor()
    cursor.execute('SELECT transaksi.id_transaksi,id_order, pelanggan.nama, daftar_mobil.merk, driver.name, transaksi.tanggal_mulai,durasi,total,status FROM transaksi join pelanggan on transaksi.id_pelanggan=pelanggan.id_pelanggan join daftar_mobil on transaksi.kode_mobil = daftar_mobil.kode_mobil left join driver on transaksi.id_driver = driver.id_driver where transaksi.status in("Transaksi selesai", "Transaksi ditolak")')
    transaksis = cursor.fetchall()
    cursor.close()
